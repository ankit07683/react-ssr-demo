...
function BlogListRow(props) {
  ...
  return (
    <div className="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
      <div className="col-xs-12 col-sm-7 p-4">
        <strong className="d-inline-block mb-2 text-success">{author}</strong>
        <h3 className="mb-0">{title}</h3>
        <div className="mb-1 text-muted">
          {moment(createdAt).format("DD-MM-YYYY")}
        </div>
        <p className="mb-auto">{meta_description}</p>

        <Link to={`${appRoutes.BLOGS}/${url}`} className="stretched-link">
          Continue reading
        </Link>
      </div>
      <div className="col-xs-12 col-sm-5 p-4">
        <img
          src={`${constants.host}/blog-images/${image}`}
